...
function buildIngameBanAppealPanel() {
  const appealButtonRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
  ...
  return {
    ...
    files: [new AttachmentBuilder(BAN_APPEAL_BANNER_PATH, { name: BAN_APPEAL_BANNER_FILENAME })],
    flags: MessageFlags.IsComponentsV2,
  };
}

export async function postIngameBanAppealPanelMessage(channel: TextChannel): Promise<void> {
