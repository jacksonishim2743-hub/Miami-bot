...
const activeGiveawayByChannel = new Map<string, string>();
const polls = new Map<string, PollState>();
const activePollByChannel = new Map<string, string>();

function canManageCommunity(member: GuildMember): boolean {
...
function roleAllowedMentions(roleId: string) {
  return { roles: [roleId] };
}

function buildGiveawayPanel(state: GiveawayState) {
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("-# Miami Roleplay © 2026 | Giveaway Operations"),
    );

  return { components: [container], flags: MessageFlags.IsComponentsV2 };
}

function buildPollPanel(state: PollState) {
  ...
  if (interaction.commandName === "giveaway") {
    ...
    if (subcommand === "end") {
      ...
      if (current.state.winnerId) {
        await interaction.channel.send(
          `Hello <@${current.state.winnerId}> you won our most recent giveaway. The prize is **${current.state.prize}**.`,
        );
      }
...
export async function handleCommunityButtonInteraction(
  ...
  let vote: PollChoice | null = null;
  if (interaction.customId === POLL_YES_BUTTON_ID) vote = "yes";
  if (interaction.customId === POLL_NO_BUTTON_ID) vote = "no";
  if (interaction.customId === POLL_UNSURE_BUTTON_ID) vote = "unsure";
