...
const afkEntries = new Map<string, AfkEntry>();

function parseAfkDuration(duration: string): number | null {
...
function isAfkExpired(entry: AfkEntry): boolean {
  return Date.now() >= entry.expiresAt;
}

function buildAfkNickname(displayName: string): string {
...
async function applyAfkNickname(member: GuildMember): Promise<void> {
...
export function isAfkCommandMessage(content: string, prefix: string): boolean {
  const trimmed = content.trim().toLowerCase();
  return (
    trimmed.startsWith(`${prefix.toLowerCase()}afk `) ||
...
export async function handleAfkReturn(message: Message): Promise<boolean> {
  if (!message.member) {
    return false;
  }
...
export async function handleAfkMentions(message: Message): Promise<void> {
  ...
  for (const [, mentionedMember] of message.mentions.members) {
    ...
    responses.push(
      `${mentionedMember} is afk please do not bother only if an emergency has occured\n**Reason:** ${entry.reason}`,
    );
  }
