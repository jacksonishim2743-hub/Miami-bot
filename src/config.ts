...
function getRequiredEnv(name: "DISCORD_TOKEN"): string {
...
export const config = {
  ...
  automod: {
    ...
    blockedWords: parseList(
      process.env.BLOCKED_WORDS ?? DEFAULT_BLOCKED_WORDS.join(","),
    ...
    allowedLinkDomains: parseList(process.env.ALLOWED_LINK_DOMAINS).map((domain) =>
      domain.toLowerCase(),
    ),
    maxMentions: parseNumber(process.env.AUTOMOD_MAX_MENTIONS, 5),
    maxWarningsBeforeTimeout: parseNumber(
      process.env.AUTOMOD_MAX_WARNINGS_BEFORE_TIMEOUT,
      3,
    ),
    warningWindowMs:
      parseNumber(process.env.AUTOMOD_WARNING_WINDOW_MINUTES, 15) * 60 * 1000,
    warningMessageLifetimeMs:
      parseNumber(process.env.AUTOMOD_WARNING_MESSAGE_SECONDS, 10) * 1000,
    timeoutDurationMs:
      parseNumber(process.env.AUTOMOD_TIMEOUT_MINUTES, 10) * 60 * 1000,
    caps: {
    ...
    spam: {
      ...
      minDuplicateLength: parseNumber(
        process.env.AUTOMOD_DUPLICATE_MIN_LENGTH,
        8,
      ),
    },
  },
