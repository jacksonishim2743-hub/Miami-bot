...
exports.postPartnershipsPanelMessage = postPartnershipsPanelMessage;
exports.handlePartnershipsSelectInteraction = handlePartnershipsSelectInteraction;
...
function buildPartnershipsPanel() {
    ...
    return {
        ...
        files: [new discord_js_1.AttachmentBuilder(PARTNERSHIPS_BANNER_PATH, { name: PARTNERSHIPS_BANNER_FILENAME })],
        flags: discord_js_1.MessageFlags.IsComponentsV2,
    };
}
function buildTierPanel(tier) {
    const header = (() => {
        switch (tier) {
            case "small":
                return [
    ...
    const container = new discord_js_1.ContainerBuilder()
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(header.join("\n")))
        .addSeparatorComponents(new discord_js_1.SeparatorBuilder())
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(requirements.join("\n")))
        .addSeparatorComponents(new discord_js_1.SeparatorBuilder())
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(reviewInfo.join("\n")), new discord_js_1.TextDisplayBuilder().setContent([
        "**Final Reminder**",
...
async function postPartnershipsPanelMessage(channel) {
    await channel.send(buildPartnershipsPanel());
}
async function handlePartnershipsSelectInteraction(interaction) {
