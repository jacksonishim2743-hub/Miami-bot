...
exports.postSideApplicationsPanelMessage = postSideApplicationsPanelMessage;
exports.handleSideApplicationsSelectInteraction = handleSideApplicationsSelectInteraction;
...
const sideApplicationMeta = {
    fishing_license: {
        label: "Fishing License",
        emoji: "🎣",
        url: "https://melon.ly/form/7464818954691678208",
        overview: "*The fishing license application is for players who want legal access to fishing-related activity under Miami Roleplay expectations. This helps keep civilian roleplay cleaner, more structured, and easier for staff to review when questions come up in-game.*",
        qualifications: [
    ...
    gun_permit: {
        ...
        overview: "*The gun permit application is for players requesting lawful weapon access under Miami Roleplay policy. Because this can directly affect scene quality, staff response, and realism, permit reviews are expected to be taken seriously.*",
        qualifications: [
    ...
    food_vendor_permit: {
        ...
        overview: "*The food vendor permit application is for players who want to operate food-based civilian roleplay in a recognized, organized way. This can include stands, vendor scenes, event-based sales, or other approved business-style interactions.*",
        qualifications: [
...
function buildSideApplicationsPanel() {
    ...
    return {
        ...
        files: [new discord_js_1.AttachmentBuilder(SIDE_APPLICATIONS_BANNER_PATH, { name: SIDE_APPLICATIONS_BANNER_FILENAME })],
        flags: discord_js_1.MessageFlags.IsComponentsV2,
    };
}
function buildSideApplicationDetailPanel(type) {
    ...
    const container = new discord_js_1.ContainerBuilder()
        ...
        .addSeparatorComponents(new discord_js_1.SeparatorBuilder())
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(["**Who This Is Best For**", ...meta.qualifications].join("\n")))
        .addSeparatorComponents(new discord_js_1.SeparatorBuilder())
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(["**Permit Expectations**", ...meta.expectations].join("\n")), new discord_js_1.TextDisplayBuilder().setContent([
        "**Final Reminders**",
        ...meta.reminders,
        "",
        "-# Miami Roleplay © 2026 | Side Applications",
...
async function postSideApplicationsPanelMessage(channel) {
    await channel.send(buildSideApplicationsPanel());
}
async function handleSideApplicationsSelectInteraction(interaction) {
