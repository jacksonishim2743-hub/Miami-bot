...
exports.ticketButtonIds = void 0;
exports.postTicketPanelMessage = postTicketPanelMessage;
exports.isTicketChannelName = isTicketChannelName;
exports.isTicketChannel = isTicketChannel;
exports.isTicketOwner = isTicketOwner;
...
exports.requestTicketClose = requestTicketClose;
exports.requestClosePanel = requestClosePanel;
exports.switchTicketCategory = switchTicketCategory;
exports.renameTicketChannel = renameTicketChannel;
exports.addUserToTicket = addUserToTicket;
exports.removeUserFromTicket = removeUserFromTicket;
exports.handleTicketMessageCreate = handleTicketMessageCreate;
exports.handleTicketButtonInteraction = handleTicketButtonInteraction;
exports.handleTicketSelectInteraction = handleTicketSelectInteraction;
...
function ticketTopic(ownerId, ticketType) {
...
function getCategoryIdFor(ticketType) {
...
function buildTopicSelect() {
    return new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
        ...
        .addOptions(mainTicketOptions.map((option) => ({
        label: option.label,
        value: option.value,
        emoji: option.emoji,
    }))));
...
function buildProductionSelect() {
    return new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
        ...
        .addOptions(productionTicketOptions.map((option) => ({
        label: option.label,
        value: option.value,
        emoji: option.emoji,
    }))));
}
function buildTicketQuestionsModal(ticketType) {
    ...
    return new discord_js_1.ModalBuilder()
        ...
        .setRequired(true)
        .setMaxLength(200)
        .setPlaceholder("Type your answer here...")), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder()
        .setCustomId("question_two")
        .setLabel(modalLabel(q2))
        .setStyle(discord_js_1.TextInputStyle.Paragraph)
        .setRequired(true)
        .setMaxLength(1000)
        .setPlaceholder("Give us the full details...")), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder()
...
function buildTicketInfoLines(ticketType) {
    switch (ticketType) {
...
function buildTicketInfoPanel(member, ticketType) {
    const container = new discord_js_1.ContainerBuilder()
        ...
        .addSeparatorComponents(new discord_js_1.SeparatorBuilder())
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(buildTicketInfoLines(ticketType).join("\n")), new discord_js_1.TextDisplayBuilder().setContent("-# Miami Roleplay © 2026 | Ticket Support"));
    return {
        allowedMentions: {
            parse: [],
            roles: [],
...
function buildTicketLifecycleLogPanel(action, openerText, ticketType, ticketName, extraLine) {
...
function buildTranscriptPanel(openerText, ticketType, ticketName, closeReasonText, answers, transcriptLines) {
    ...
    return buildTicketNoticePanel(`${ticketLabelFor(ticketType)} Transcript`, [
        `**Ticket** - \`${ticketName}\``,
        `**Opened By** - ${openerText}`,
        `**Support Type** - ${ticketLabelFor(ticketType)}`,
        closeReasonText,
...
async function sendTicketOpenedLog(member, channel, ticketType) {
    const logChannel = resolveGuildTextChannel(member, TICKET_LOG_CHANNEL_ID);
    if (!logChannel) {
        return;
    }
    await logChannel.send(buildTicketLifecycleLogPanel("opened", formatTicketUserLabel(member.user), ticketType, channel.name, "**Status** - *Ticket created successfully.*")).catch(() => null);
}
async function sendTicketCloseOutputs(channel, meta, closeReason) {
    ...
    if (logChannel?.isTextBased() && "send" in logChannel) {
        const trackedLogReason = (0, reasontracker_js_1.buildTrackedReasonField)("Close Reason", normalizedCloseReason);
        const sentLogMessage = await logChannel.send(buildTicketLifecycleLogPanel("closed", openerText, meta.ticketType, channel.name, trackedLogReason.text)).catch(() => null);
        if (sentLogMessage) {
            (0, reasontracker_js_1.finalizeTrackedReason)(trackedLogReason.entryId, guild.id, sentLogMessage.channelId, sentLogMessage.id, "Close Reason");
        }
    }
    if (transcriptChannel?.isTextBased() && "send" in transcriptChannel) {
        const trackedTranscriptReason = (0, reasontracker_js_1.buildTrackedReasonField)("Close Reason", normalizedCloseReason);
        const sentTranscriptMessage = await transcriptChannel.send(buildTranscriptPanel(openerText, meta.ticketType, channel.name, trackedTranscriptReason.text, meta.answers, transcriptLines)).catch(() => null);
        if (sentTranscriptMessage) {
            (0, reasontracker_js_1.finalizeTrackedReason)(trackedTranscriptReason.entryId, guild.id, sentTranscriptMessage.channelId, sentTranscriptMessage.id, "Close Reason");
        }
    }
    if (ownerMember) {
        await ownerMember.send(buildTicketClosedDmPanel(guild.id, meta.ticketType, channel.name, plainCloseReasonText)).catch(() => null);
    }
}
function buildTicketAnswersPanel(ticketType, answers) {
    const questions = ticketQuestions[ticketType];
    const answerLines = questions.flatMap((question, index) => [
        `**${question}**`,
        `*${answers[index] || "No response provided"}*`,
    ...
    const container = new discord_js_1.ContainerBuilder()
        ...
        "",
        ...answerLines,
    ].join("\n").trim()), new discord_js_1.TextDisplayBuilder().setContent("-# Miami Roleplay © 2026 | Ticket Support"));
    return {
        allowedMentions: {
            parse: [],
            roles: [],
...
function buildTicketNoticePanel(title, lines, footer = "-# Miami Roleplay © 2026 | Ticket Operations") {
    const container = new discord_js_1.ContainerBuilder()
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent([
        `## ${title}`,
        "",
        ...
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(footer));
    return {
        allowedMentions: {
            parse: [],
            roles: [],
...
function buildTicketClosedDmPanel(guildId, ticketType, ticketName, closeReasonText) {
    const container = new discord_js_1.ContainerBuilder()
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent([
        "## Ticket Closed",
        "",
        ...
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent("-# Miami Roleplay © 2026 | Ticket Support"));
    return {
        allowedMentions: {
            parse: [],
            roles: [],
...
function buildCloseReasonPromptPanel(remainingSeconds) {
...
function buildCloseRequestPanel(requestedByText, reasonText, statusText = "*Waiting for a response from the ticket opener.*", includeButtons = true) {
    const container = new discord_js_1.ContainerBuilder()
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent([
        "## Close Request",
        "",
    ...
    container.addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent("-# Miami Roleplay © 2026 | Ticket Operations"));
    return {
        components: [container],
        flags: discord_js_1.MessageFlags.IsComponentsV2,
    };
...
function buildTicketPanel() {
    const openButtonRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
    ...
    const container = new discord_js_1.ContainerBuilder()
        ...
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent("-# Miami Roleplay © 2026 | Ticket Operations"));
    return {
        components: [container],
        files: [new discord_js_1.AttachmentBuilder(TICKET_PANEL_BANNER_PATH, { name: TICKET_PANEL_BANNER_FILENAME })],
        flags: discord_js_1.MessageFlags.IsComponentsV2,
    };
}
...
function canCloseTicket(channel, member) {
    ...
    if (member.roles.cache.has(FOUNDERSHIP_ROLE_ID) ||
        member.permissions.has(discord_js_1.PermissionFlagsBits.ManageChannels) ||
...
async function createTicketChannel(member, ticketType, answers) {
...
async function closeTicketChannel(channel, reason = "No reason provided") {
    const meta = getTicketMeta(channel);
    if (meta) {
        try {
...
function normalizeCloseReason(content) {
...
async function runCloseCountdown(channel, closingUserId, reminderMessage) {
    ...
    const countdownTimer = setInterval(() => {
        ...
        void reminderMessage.edit(buildCloseReasonPromptPanel(remainingSeconds)).catch(() => null);
    }, CLOSE_COUNTDOWN_INTERVAL_MS);
    try {
        const collected = await channel.awaitMessages({
            filter: (message) => message.author.id === closingUserId &&
                !message.author.bot &&
...
async function requestTicketClose(channel, closer, reason) {
    if (pendingTicketClosures.get(channel.id)) {
        await channel.send(`${closer}, a ticket close request is already waiting for a reason in this channel.`);
        return;
    }
    ...
    try {
        ...
        if (!closeReason) {
            const reminderMessage = await channel.send(buildCloseReasonPromptPanel(60));
            const collectedReason = await runCloseCountdown(channel, closer.id, reminderMessage);
            if (!collectedReason) {
                await reminderMessage.edit(buildTicketNoticePanel("Ticket Closure Expired", [
...
async function requestClosePanel(channel, closer, reason) {
    const meta = getTicketMeta(channel);
    if (!meta) {
        return;
    }
    ...
    const requestMessage = await channel.send(buildCloseRequestPanel(`${closer}`, trackedReason.text));
    closeRequestStates.set(requestMessage.id, {
        requesterId: closer.id,
        ownerId: meta.ownerId,
        reason,
    });
    (0, reasontracker_js_1.finalizeTrackedReason)(trackedReason.entryId, channel.guild.id, requestMessage.channelId, requestMessage.id, "Reason");
    await requestMessage.pin().catch(() => null);
}
async function switchTicketCategory(channel, categoryId) {
    await channel.setParent(categoryId, { lockPermissions: false });
}
async function renameTicketChannel(channel, nextName) {
...
async function addUserToTicket(channel, userIdInput) {
...
async function removeUserFromTicket(channel, userIdInput, reason) {
    const userId = extractUserId(userIdInput);
    if (!userId) {
        throw new Error("Please provide a valid user ID.");
    }
    ...
    await member.send(`You were removed from ${channel.guild.name}'s ticket \`${channel.name}\`.\nReason: ${reason || "No reason provided."}`).catch(() => null);
    return member.toString();
}
async function handleTicketMessageCreate(message) {
    if (!(message.channel instanceof discord_js_1.TextChannel) || !isTicketChannel(message.channel)) {
        return;
    }
    ...
    if (message.mentions.everyone ||
        ...
        await message.channel.send(buildTicketNoticePanel("Ticket Closed Automatically", [
            "*This ticket was closed because the ticket opener pinged another user or role inside the ticket.*",
        ])).catch(() => null);
        await closeTicketChannel(message.channel, "Ticket opener pinged another user or role in the ticket.");
    }
}
async function handleTicketButtonInteraction(interaction) {
    ...
    if (interaction.customId === "ticket:close-request:approve" ||
        ...
        if (!interaction.channel || !(interaction.channel instanceof discord_js_1.TextChannel)) {
            await interaction.reply({ content: "This close request is no longer valid.", flags: discord_js_1.MessageFlags.Ephemeral });
            return true;
        }
        ...
        if (interaction.customId === "ticket:close-request:cancel") {
            if (interaction.user.id !== state.ownerId) {
                await interaction.reply({ content: "Only the ticket opener can cancel this close request.", flags: discord_js_1.MessageFlags.Ephemeral });
                return true;
            }
            ...
            await interaction.update(buildCloseRequestPanel(`<@${state.requesterId}>`, state.reason, "*The ticket opener denied this close request.*", false));
        ...
        if (!interaction.member || !canCloseTicket(interaction.channel, interaction.member)) {
            await interaction.reply({ content: "Only staff can close this ticket.", flags: discord_js_1.MessageFlags.Ephemeral });
            return true;
        }
        await interaction.update(buildCloseRequestPanel(`<@${state.requesterId}>`, state.reason, "*This close request was approved and the ticket is closing.*", false));
        closeRequestStates.delete(interaction.message.id);
...
async function handleTicketModalSubmit(interaction) {
    ...
    const ticketType = interaction.customId.slice(TICKET_MODAL_PREFIX.length);
    const answers = [
