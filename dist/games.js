...
exports.gameCommandLines = void 0;
exports.handleGameCommand = handleGameCommand;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const discord_js_1 = require("discord.js");
const SETTINGS_PATH = node_path_1.default.resolve(process.cwd(), "game-settings.json");
const hangmanStates = new Map();
...
function saveGameChannels() {
    node_fs_1.default.writeFileSync(SETTINGS_PATH, JSON.stringify(configuredChannels, null, 2), "utf8");
}
function normalizeAnswer(value) {
    return value.trim().toLowerCase().replace(/\s+/g, " ");
}
function getConfiguredChannelId(game) {
    return configuredChannels[game];
}
function isAllowedGameChannel(message, game) {
    return message.inGuild() && getConfiguredChannelId(game) === message.channelId;
}
async function ensureAllowedGameChannel(message, game) {
...
async function collectReply(message, timeoutMs = 20_000) {
    const collected = await message.channel.awaitMessages({
        filter: (reply) => !reply.author.bot &&
...
function resolveTargetTextChannel(message) {
    const mentioned = message.mentions.channels.first();
    if (mentioned instanceof discord_js_1.TextChannel) {
        return mentioned;
    }
    return message.channel instanceof discord_js_1.TextChannel ? message.channel : null;
}
async function setGameChannel(message, game) {
    if (!message.member?.permissions.has(discord_js_1.PermissionsBitField.Flags.ManageChannels)) {
...
async function showGameChannels(message) {
    const lines = Object.keys(gameDefinitions).map((game) => {
...
async function showGames(message) {
    const lines = Object.keys(gameDefinitions).map((game) => `• **${gameDefinitions[game].label}** — ${gameDefinitions[game].playCommand}`);
    await message.reply(`Available games:\n${lines.join("\n")}`);
    return true;
}
async function testGame(message, args) {
    const key = normalizeAnswer(args.join(" "));
    ...
    await message.reply(`Test **${game.label}** with ${game.testCommand} and play it with ${game.playCommand}. Set its channel first with ${game.setupCommand}.`);
    return true;
}
async function playTrivia(message) {
    if (!(await ensureAllowedGameChannel(message, "trivia")))
        return true;
    ...
    await message.reply(normalizeAnswer(reply.content) === item.answer
        ? "✅ Correct answer!"
        : `❌ Not quite. The answer was **${item.answer}**.`);
    return true;
}
async function playScramble(message) {
    if (!(await ensureAllowedGameChannel(message, "scramble")))
        return true;
    ...
    await message.reply(normalizeAnswer(reply.content) === word
        ? "✅ You unscrambled it!"
        : `❌ The word was **${word}**.`);
    return true;
}
async function playMathChallenge(message) {
    if (!(await ensureAllowedGameChannel(message, "mathchallenge")))
        return true;
    ...
    await message.reply(Number(reply.content.trim()) === answer
        ? "✅ Correct answer!"
        : `❌ The correct answer was **${answer}**.`);
    return true;
}
async function playHangman(message, args) {
    if (!(await ensureAllowedGameChannel(message, "hangman")))
        return true;
...
async function playTrueFalse(message) {
    if (!(await ensureAllowedGameChannel(message, "truefalse")))
        return true;
    ...
    await message.reply(guessed === item.answer
        ? "✅ Correct!"
        : `❌ The correct answer was **${item.answer ? "true" : "false"}**.`);
    return true;
}
async function playQuickdraw(message) {
    if (!(await ensureAllowedGameChannel(message, "quickdraw")))
        return true;
...
async function playMemory(message) {
    if (!(await ensureAllowedGameChannel(message, "memory")))
        return true;
    ...
    await message.reply(reply.content.trim() === sequence.join(" ")
        ? "✅ Perfect memory!"
        : `❌ The sequence was **${sequence.join(" ")}**.`);
    return true;
}
async function playRiddle(message) {
    if (!(await ensureAllowedGameChannel(message, "riddle")))
        return true;
...
async function playTypingRace(message) {
    if (!(await ensureAllowedGameChannel(message, "typingrace")))
        return true;
    ...
    await message.reply(winner.content.trim() === prompt
        ? `🏁 ${winner.author} won the typing race!`
        : `❌ That did not match exactly. The prompt was **${prompt}**.`);
    return true;
}
async function playWouldYouRather(message) {
    if (!(await ensureAllowedGameChannel(message, "wouldyourather")))
        return true;
...
async function handleGameCommand(message, rawContent) {
    const [commandName, ...args] = rawContent.split(/\s+/);
    const command = commandName.toLowerCase();
    if (command === "set-trivia-channel")
        return setGameChannel(message, "trivia");
    if (command === "set-scramble-channel")
        return setGameChannel(message, "scramble");
    if (command === "set-math-channel")
        return setGameChannel(message, "mathchallenge");
    if (command === "set-hangman-channel")
        return setGameChannel(message, "hangman");
    if (command === "set-truefalse-channel")
        return setGameChannel(message, "truefalse");
    if (command === "set-quickdraw-channel")
        return setGameChannel(message, "quickdraw");
    if (command === "set-memory-channel")
        return setGameChannel(message, "memory");
    if (command === "set-riddle-channel")
        return setGameChannel(message, "riddle");
    if (command === "set-typingrace-channel")
        return setGameChannel(message, "typingrace");
    if (command === "set-wouldyourather-channel")
        return setGameChannel(message, "wouldyourather");
    if (command === "game-channels")
        return showGameChannels(message);
    if (command === "games")
        return showGames(message);
    if (command === "testgame")
        return testGame(message, args);
    if (command === "trivia")
        return playTrivia(message);
    if (command === "scramble")
        return playScramble(message);
    if (command === "mathchallenge")
        return playMathChallenge(message);
    if (command === "hangman")
        return playHangman(message, args);
    if (command === "truefalse")
        return playTrueFalse(message);
    if (command === "quickdraw")
        return playQuickdraw(message);
    if (command === "memory")
        return playMemory(message);
    if (command === "riddle")
        return playRiddle(message);
    if (command === "typingrace")
        return playTypingRace(message);
    if (command === "wouldyourather")
        return playWouldYouRather(message);
    return false;
}
